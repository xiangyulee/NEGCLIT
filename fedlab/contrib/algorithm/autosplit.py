...
class AutoSplitServerHandler(SyncServerHandler):
    ...
    def __init__(self, model: torch.nn.Module, global_round: int, sample_ratio: float, cuda: bool = True, device: str = None, logger: Logger = None, args=None):
        super(AutoSplitServerHandler, self).__init__(model, global_round,sample_ratio,cuda, device,logger)
        ...
        _model=model_name[args.model](nclasses=dataset_class_num[args.offline_dataset],split_layer=net['split_layer'],cfg=net['cfg']) 
